...
/**
 * Using 'gemini-3-flash-preview' as it provides the highest request capacity 
 * for basic text and information retrieval tasks. 
 * Note: Google Maps grounding is removed as it is only compatible with the 2.5 series.
 */
...
export const fetchCityData = async (
  ...
  try {
    const response: GenerateContentResponse = await withRetry(() => ai.models.generateContent({
      ...
      config: {
        // googleSearch is used for grounding. 
        // googleMaps is removed as it requires Gemini 2.5 series.
        tools: [{ googleSearch: {} }],
      },
    }));
